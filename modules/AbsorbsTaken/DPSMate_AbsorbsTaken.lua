...
DPSMate.Modules.AbsorbsTaken.Events = {
			...
			if ca ~= "i" then
				...
				for c, v in pairs(va) do -- 1
					...
					for ce, ve in pairs(v) do
						if ce ~= "i" then
							...
							for qq, ss in ve do
								...
								if not temp[ce][qq] then
									temp[ce][qq] = ss * p
								else
									temp[ce][qq] = temp[ce][qq] + ss * p
								end
							end
						end
					end
					if v["i"][1] == 1 then
						...
						if not temp[v["i"][4]] then
							temp[v["i"][4]] = {}
						end
						if not temp[v["i"][4]][v["i"][3]] then
							temp[v["i"][4]][v["i"][3]] = v["i"][2]
						else
							temp[v["i"][4]][v["i"][3]] = temp[v["i"][4]][v["i"][3]] + v["i"][2]
						end
					end
					PerAbilityAbsorb = PerAbilityAbsorb + PerShieldAbsorb
				end
...
function DPSMate.Modules.AbsorbsTaken:GetSettingValues(arr, cbt, k, ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMateSettings["columnsabsorbstaken"][1] then
			str[1] = " " .. DPSMate:Commas(va, k) .. p; strt[2] = DPSMate:Commas(tot, k) .. pt
		end
		if DPSMateSettings["columnsabsorbstaken"][2] then
			str[2] = " (" .. strformat("%.1f", 100 * varea / totr) .. "%)"
		end
		if DPSMateSettings["columnsabsorbstaken"][3] then
			str[3] = " (" .. strformat("%.1f", va / cbt) .. p .. ")"
		end
		if DPSMateSettings["columnsabsorbstaken"][4] then
			str[4] = " (" .. strformat("%.1f", va / (ecbt[pname] or cbt)) .. p .. ")"
		end
		tinsert(name, pname)
