			...
			for ca, va in pairs(val) do
				if ca ~= "i" then
					...
					while true do
						if (not tb[i]) then
							tinsert(ta, i, ca)
							tinsert(tb, i, (va[13] or 0) / cbt)
							break
						else
							if (tb[i] < (va[13] or 0) / cbt) then
								tinsert(ta, i, ca)
								tinsert(tb, i, (va[13] or 0) / cbt)
								break
							end
...
function DPSMate.Modules.DTPS:GetSettingValues(arr, cbt, k, ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMateSettings["columnsdtps"][2] then
			str[1] = " " .. strformat("%.1f", dmg / cbt) .. p; strt[2] = " " .. strformat("%.1f", (tot / cbt)) .. pt
		end
		if DPSMateSettings["columnsdtps"][3] then str[2] = " (" .. strformat("%.1f", 100 * dmgr / totr) .. "%)" end
		if DPSMateSettings["columnsdtps"][1] then
			str[3] = "(" .. DPSMate:Commas(dmg, k) .. p .. ")"; strt[1] = "(" .. DPSMate:Commas(tot, k) .. pt .. ")"
		end
