...
function DPSMate.Modules.EDT:GetSettingValues(arr, cbt, k, ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMateSettings["columnsedt"][1] then
		...
		if DPSMateSettings["columnsedt"][2] then
			str[3] = "(" .. strformat("%.1f", dmg / cbt) .. p .. ")"; strt[1] = "(" .. strformat("%.1f", (tot / cbt)) .. pt .. ")"
		end
