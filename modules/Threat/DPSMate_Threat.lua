...
function DPSMate.Modules.Threat:GetSettingValues(arr, cbt, k, ecbt)
	...
	for cat, val in sortedTable do
		...
		local str = { [1] = "", [2] = "", [3] = "", [4] = "" }
		if DPSMateSettings["columnsthreat"][1] then
			str[1] = " " .. strformat("%.2f", dmg) .. p; strt[2] = strformat("%.2f", tot) .. p
		end
		if DPSMateSettings["columnsthreat"][2] then
			str[2] = "(" .. strformat("%.1f", (dmg / cbt)) .. p .. ")"; strt[1] = "(" .. strformat("%.1f", (tot / cbt)) .. pt .. ") "
		end
