...
DPSMate.Modules.Overhealing.Events = {
...
function DPSMate.Modules.Overhealing:GetSettingValues(arr, cbt, k, ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMateSettings["columnsoverhealing"][1] then
		...
		if DPSMateSettings["columnsoverhealing"][2] then
			str[3] = "(" .. strformat("%.1f", va / cbt) .. ")"; strt[1] = "(" .. strformat("%.1f", (tot / cbt)) .. pt .. ")"
		end
