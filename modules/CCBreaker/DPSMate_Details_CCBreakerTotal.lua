...
function DPSMate.Modules.DetailsCCBreakerTotal:UpdateBuffs(arg1)
	...
	for i = 1, 6 do
		local pos = Buffpos + i
		if not DetailsArr[pos] then
			break
		end
		local userIndex = DetailsArr[pos][3];
		_G(path .. i).user = DetailsArr[pos][6]
		_G(path .. i .. "_Icon"):SetTexture(DetailsArr[pos][2])
		_G(path .. i .. "_Ability"):SetText(DetailsArr[pos][1])
		_G(path .. i .. "_Target"):SetText("|cFF" .. hexClassColor[(DPSMateUser[userIndex] and DPSMateUser[userIndex][2]) or "warrior"] .. userIndex .. "|r")
